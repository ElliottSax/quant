...
export const api = {
  ...
  backtesting: {
    run: (req: BacktestRequest) =>
      // Use demo endpoint (no auth required) - TODO: Add authenticated endpoint for premium features
    ...
    strategies: () =>
      // Use demo endpoint (no auth required) - returns free strategies only
      fetchAPI<StrategyInfo[]>('/backtesting/demo/strategies'),
