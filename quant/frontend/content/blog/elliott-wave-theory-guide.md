...
- **Wave 1**: Initial move in the trend direction. Often occurs while sentiment is still bearish (for uptrends). Typically the shortest impulse wave.
...
1. **Wave 2 cannot retrace more than 100% of Wave 1.** If it does, the supposed Wave 1 is not actually a Wave 1.
2. **Wave 3 cannot be the shortest of the three impulse waves (1, 3, 5).** Wave 3 is usually the longest, but it only needs to not be the shortest.
3. **Wave 4 cannot enter the price territory of Wave 1.** In an uptrend, the low of Wave 4 must stay above the high of Wave 1.

These three rules are absolute. Every other principle in Elliott Wave analysis is a guideline (probable but not guaranteed).
...
1. Identify a completed five-wave decline or three-wave correction at a higher degree
2. Identify Wave 1 (the initial impulse in the new trend direction)
3. Wait for Wave 2 to retrace to the 50-61.8% Fibonacci level of Wave 1
4. Enter long when a confirmation signal appears at the Wave 2 low (candlestick pattern, momentum divergence)
5. Stop-loss below the Wave 1 starting point (Wave 2 cannot exceed 100% of Wave 1)
6. Target: 161.8% extension of Wave 1

This setup offers excellent risk/reward because the stop is defined by the cardinal rule (Wave 2 cannot exceed Wave 1), and Wave 3 is typically the strongest wave.
...
**Mistake 1: Forcing wave counts.** The most common error is fitting the wave count to a preexisting bias. If the count requires bending or breaking rules, it is wrong. Accept the ambiguity and wait for the count to clarify.

**Mistake 2: Ignoring multiple valid counts.** At any given time, multiple wave counts may be valid. Professional Elliott Wave analysts always maintain a preferred count and at least one alternative count. They define the price level that would invalidate each count, creating objective decision points.

**Mistake 3: Counting on timeframes that are too low.** Elliott Wave patterns are fractal, but noise on very short timeframes makes accurate counting extremely difficult. Start with daily or weekly charts and only use lower timeframes for entry timing once the higher timeframe count is established.
